...
check = pufs_export_wrapped_key(
    SSKEY, CLIENT_KEY_SLOT, out, // CLIENT_KEY_SLOT is the AES key slot
...
sequenceDiagram
    participant User
    participant APP_Board as APP Board (Client)
    participant PUF as PUFse
    participant Server as Backup Board (Server)
    participant File as Backup File

    User->>APP_Board: Run ./client -a SERVER_IP -p SERVER_PORT -c PASSWD
    APP_Board->>Server: Establish TLS connection
    APP_Board->>Server: ECDH key exchange (derive shared secret)
    APP_Board->>PUF: Derive KEK using pufs_kdf (PUF + shared secret)
    APP_Board->>PUF: Export & wrap AES key with KEK (pufs_export_wrapped_key)
    APP_Board->>Server: Send wrapped AES key + HMAC
    Server->>File: Store wrapped key as <MAC>_<PASSWD_HASH>.bin
    Server-->>APP_Board: Send backup OK
    APP_Board-->>User: Print "key backup OK"
...
sequenceDiagram
    participant User
    participant New_Board as New Board (Client)
    participant PUF as PUFse
    participant Server as Backup Board (Server)
    participant File as Backup File

    User->>New_Board: Run ./client -a SERVER_IP -p SERVER_PORT -c PASSWD -m MAC_ADDR -r
    New_Board->>Server: Establish TLS connection
    New_Board->>Server: Request wrapped AES key file (using MAC_ADDR & PASSWD)
    Server->>File: Read wrapped key file <MAC>_<PASSWD_HASH>.bin
    Server->>New_Board: Send wrapped AES key + HMAC
    New_Board->>PUF: Derive KEK using pufs_kdf (PUF + shared secret)
    New_Board->>PUF: Unwrap AES key with KEK (pufs_import_wrapped_key)
    New_Board-->>User: Print "Restore OK" (if successful)
```